...
!
!       Outputs:
!               Permittivity : complex dielectric constant of sea water
!
!
!       E. Dinnat
...
!
! NAME:
!       MW_Ocean_Permittivity
!
! CALLING SEQUENCE:
!       CALL MW_Ocean_Permittivity( Temperature , & ! Input
!                                        Salinity    , & ! Input
!                                        Frequency   , & ! Input
!                                        Permittivity, & ! Output
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Meissner_Ocean_Permittivity( &
    Temperature , & ! Input
    Salinity    , & ! Input
    Frequency   , & ! Input
    Permittivity  ) ! Output
    ...
    REAL*8, PARAMETER :: ai(11)=(/ 5.7230D+00, 2.2379D-02, -7.1237D-04, 5.0478D+00, -7.0315D-02,&
                                6.0059D-04,3.6143D+00, 2.8841D-02, 1.3652D-01,1.4825D-03,2.4166D-04 /)
     
    REAL*8, PARAMETER :: bi(13)=(/-3.56417D-03,  4.74868D-06,1.15574D-05, 2.39357D-03, -3.13530D-05,&
                                2.52477D-07,-6.28908D-03, 1.76032D-04, -9.22144D-05, -1.99723D-02,&
                                1.81176D-04, -2.04265D-03,  1.57883D-04/)
    
    REAL*8, PARAMETER :: bi_new1(3)=(/ -0.33330D-02,  4.74868D-06, 0.0D0/)
    
    REAL*8, PARAMETER :: bi_new2(5)=(/0.23232D-02, -0.79208D-04, 0.36764D-05, -0.35594D-06, 0.89795D-08/)
    REAL*8 :: fGHz,f, f0, SST,SST2, SST3, SST4, SSS,SSS2
    REAL*8 :: es, e1, einf, nu1,nu2 ! Fresh water parameters
    REAL*8 :: sigma35, R15, alpha0, alpha1, RTR15
    ...
    SST = Temperature - K_TO_C
    SSS = Salinity
    f = Frequency
    
    if (SST.le.-40.D0) then

        print *, 'Error in MW. SST is lower than -40 degree C.'
        error stop
      ...
      if (SST.ge.34.D0) then
        ...
        print *, 'Error in MW. SST is larger than +34 degree C.'
        error stop
      ...
      if (SSS.ge.40.D0) then
        ...
        print *, ' Error in MW. SSS is larger than +40 psu.'
        error stop
      ...
      if (SSS.lt.0.D0) then
        ...
        print *, ' Error in MW. SSS is negative.'
        error stop
      ...
      sigma35 = 2.903602D0 + 8.60700D-2*SST + 4.738817D-4*SST2 - 2.9910D-6*SST3 + 4.3047D-9*SST4
      R15 = SSS*( 37.5109D0 +5.45216D0*SSS +1.4409D-2*SSS2)/(1004.75D0+182.283D0*SSS+SSS2 )
      alpha0 = ( 6.9431D0 + 3.2841D0*SSS - 9.9486D-2*SSS2 ) / (84.850D0 + 69.024D0*SSS + SSS2 )
      ...
      es_s = es *exp ( bi_new1(1)*SSS + bi_new1(2)*SSS2 + bi_new1(3)*SSS*SST ) 
...
!     c3 = 1.0D0 + SSS*(bi(10) + bi(11)*SST) ! Expression for c3  amended in Meissner et al. 2016
      ...
      Permittivity=(es_s-e1_s)/(1.0D0-j*(fGHz/nu1_s))+(e1_s-einf_s)/(1.0D0-j*(fGHz/nu2_s))+einf_s+j*sigma*f0/fGHz
      Permittivity = CONJG(Permittivity)
  END SUBROUTINE Meissner_Ocean_Permittivity
